"""
Final Optimized Ensemble Training + Comprehensive Visualization
Uses best parameters from completed Optuna optimization
"""
import torch
import numpy as np
import pickle
...
print("="*80)
print("FINAL OPTIMIZED ENSEMBLE + VISUALIZATION")
print("="*80)

# Best parameters from Trial 22 (73.6% at ±1 min)
best_params = {
    'xgb_n_estimators': 7408, 'xgb_max_depth': 4, 'xgb_lr': 0.0035127053251087855,
    'xgb_subsample': 0.9427084653257143, 'xgb_colsample': 0.9009758665266883,
    'xgb_colsample_level': 0.8957090771543987, 'xgb_min_child': 6,
    'xgb_gamma': 2.2684706454709342, 'xgb_alpha': 3.9127857880553463,
    'xgb_lambda': 1.2521822750549703, 'xgb_max_delta': 2,
    
    'lgb_n_estimators': 5643, 'lgb_num_leaves': 138, 'lgb_lr': 0.015027042762414636,
    'lgb_feature_fraction': 0.5261649784427219, 'lgb_bagging_fraction': 0.9952081670733998,
    'lgb_bagging_freq': 3, 'lgb_max_depth': 20, 'lgb_min_child': 25,
    'lgb_min_child_weight': 2.3984011925701307, 'lgb_alpha': 0.7199333349272936,
    'lgb_lambda': 2.9574514203777866, 'lgb_min_split_gain': 2.9893133374645817,
    
    'cat_iterations': 7349, 'cat_depth': 11, 'cat_lr': 0.029251193500648656,
    'cat_l2': 8.820784629317021, 'cat_border': 132,
    'cat_bagging_temp': 0.27848710551516764, 'cat_random_strength': 5.560986104835045,
    
    'meta_type': 'xgb_meta', 'meta_xgb_n_estimators': 238,
    'meta_xgb_max_depth': 5, 'meta_xgb_lr': 0.21588264552767597
}
...
cat_final.fit(X_train, y_train, eval_set=(X_val, y_val), 
...
cat_train_pred = cat_final.predict(X_train)
X_train_meta = np.column_stack([xgb_train_pred, lgb_train_pred, cat_train_pred])
train_ensemble_pred = meta_final.predict(X_train_meta)

# Val
xgb_val_pred = xgb_final.predict(X_val)
lgb_val_pred = lgb_final.predict(X_val)
cat_val_pred = cat_final.predict(X_val)
X_val_meta = np.column_stack([xgb_val_pred, lgb_val_pred, cat_val_pred])
val_ensemble_pred = meta_final.predict(X_val_meta)
...
cat_acc = (np.abs(y_test - cat_test_pred) <= 60).mean() * 100
ens_acc = test_metrics['acc_1min']
accuracies = [xgb_acc, lgb_acc, cat_acc, ens_acc]
...
plt.tight_layout()
plt.savefig('optimized_ensemble_results.png', dpi=300, bbox_inches='tight')
print("  Saved: optimized_ensemble_results.png")

# Save models
print("\n💾 Saving models...")
xgb_final.save_model('xgboost_optimized_final.json')
lgb_final.booster_.save_model('lightgbm_optimized_final.txt')
cat_final.save_model('catboost_optimized_final.cbm')

with open('optimized_ensemble_final.pkl', 'wb') as f:
    pickle.dump({
